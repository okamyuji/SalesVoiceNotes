...
// MARK: - App Permission Configuration Tests

/// プライバシー権限のInfo.plist設定を検証するテスト
struct AppPermissionConfigurationTests {
    @Test("Info.plistにマイク利用説明が設定されている")
    func microphoneUsageDescriptionExists() throws {
    ...
    @Test("Info.plistに音声認識利用説明が設定されている")
    func speechRecognitionUsageDescriptionExists() throws {
        let plist = try loadAppInfoPlist()
...
struct VocabularyLoaderTests {
    ...
    func loadAllReturnsNonEmptyArray() {
    ...
    func loadCategoryWithSpecialCharacters() {
...
struct AudioProcessingErrorTests {
    ...
    func unsupportedAudioFormatError() {
    ...
    func recognizerUnavailableError() {
    ...
    func onDeviceNotSupportedError() {
    ...
    func recognitionFailedWithEmptyMessage() {
    ...
    func recognitionFailedWithLongMessage() {
    ...
    func recognitionFailedWithSpecialCharacters() {
...
// MARK: - Realtime Transcription Text Tests

struct RealtimeTranscriptionTextTests {
    @Test("認識トークンを連結してリアルタイム表示用テキストを作る")
    func joinRecognizedTokens() {
        let result = AudioProcessingService.joinRecognizedTokens([
            " こんにちは",
            " ",
            "世界 ",
            "！",
        ...
        #expect(result == "こんにちは世界！")
    }

    @Test("空トークンのみの場合は空文字を返す")
    func joinRecognizedTokensWithEmptyInput() {
        let result = AudioProcessingService.joinRecognizedTokens([" ", "\n", "\t"])
        #expect(result.isEmpty)
    }
}
...
struct TextFormattingTests {
    ...
    func combineWithoutPunctuation() {
    ...
    @Test("句点で文が分割される")
    func formatWithMultipleSentences() {
    ...
    func formatRemovesDuplicateNewlines() {
